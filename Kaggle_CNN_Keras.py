...
def plot_pr(labels, prediction_scores):
    fpr, tpr, _ = metrics.precision_recall_curve(labels, prediction_scores, pos_label=1)
   
    plt.plot(fpr, tpr)
    plt.ylabel('Precision')
    plt.xlabel('Recall')
    plt.legend()
    plt.title('PR Curve for cNN using Keras with Tensorflow')
    plt.tight_layout()
    plt.show()
...
plot_pr(labels, prediction_scores)
...
if produce_submission:
    ...
    plot_roc(ids, test_scores)
    plot_pr(ids, test_scores)
